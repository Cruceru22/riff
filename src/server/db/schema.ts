...
// type Artist = InferModel<typeof artists, 'insert'>;
// type Album = InferModel<typeof albums, 'insert'>;
// type Song = InferModel<typeof songs, 'insert'>;

// async function insertData() {
//   for (const artist of data) {
//     const insertedArtist = await db.insert(artists).values({ name: artist.name }).returning('id').then(rows => rows[0]?.id);
//     if (typeof insertedArtist !== 'number') {
//       throw new Error("Failed to insert artist or retrieve artist ID");
//     }

//     for (const album of artist.albums) {
//       const insertedAlbum = await db.insert(albums).values({
//         title: album.title,
//         description: album.description,
//         artistId: insertedArtist,
//       }).returning('id').then(rows => rows[0]?.id);
//       if (typeof insertedAlbum !== 'number') {
//         throw new Error("Failed to insert album or retrieve album ID");
//       }
